...
const email = "buyer@secondhandcell.com";
const password = "GPrs1234";
const displayName = "Wholesale Buyer";
...
async function ensureBuyerUser() {
  ...
  try {
    ...
    if (error.code === "auth/user-not-found") {
      console.log(`Creating buyer user ${email}…`);
  ...
  await auth.setCustomUserClaims(user.uid, { role: "buyer" });
  console.log("Applied buyer custom claim.");
  ...
  console.log(`Buyer profile saved to users/${user.uid}.`);
}

ensureBuyerUser()
  .then(() => {
    console.log("Finished provisioning buyer user.");
    process.exit(0);
  })
  .catch((error) => {
    console.error("Failed to provision buyer user:", error);
    process.exit(1);
  });
