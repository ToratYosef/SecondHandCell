...
export interface QuoteFormInitialValues {
  storage?: string;
  carrier?: string;
  screenFunctional?: boolean;
  hasCracks?: boolean;
  cosmetic?: DeviceCondition;
  imei?: string;
  shippingPreference?: 'kit' | 'self-ship';
  shippingInfo?: Partial<SubmitOrderRequest['shippingInfo']>;
  payment?: Partial<SubmitOrderRequest['payment']> & { payoutEmail?: string };
  notes?: string;
}
...
export function QuoteForm({ devicePreset, initialValues, onSubmit, loading }: QuoteFormProps) {
  ...
  const [screenFunctional, setScreenFunctional] = useState(
    initialValues?.screenFunctional ?? true
  );
  const [hasCracks, setHasCracks] = useState(initialValues?.hasCracks ?? false);
  const [cosmetic, setCosmetic] = useState<DeviceCondition>(
    initialValues?.cosmetic ?? DeviceCondition.Flawless
  );
  ...
  const [shippingInfo, setShippingInfo] = useState<SubmitOrderRequest['shippingInfo']>({
    name: initialValues?.shippingInfo?.name ?? '',
    email: initialValues?.shippingInfo?.email ?? '',
    phone: initialValues?.shippingInfo?.phone ?? '',
    address: initialValues?.shippingInfo?.address ?? '',
    city: initialValues?.shippingInfo?.city ?? '',
    state: initialValues?.shippingInfo?.state ?? '',
    postalCode: initialValues?.shippingInfo?.postalCode ?? '',
    country: initialValues?.shippingInfo?.country ?? 'US'
  });
  const [payment, setPayment] = useState<SubmitOrderRequest['payment']>({
    method: initialValues?.payment?.method ?? PaymentMethod.PayPal,
    email: initialValues?.payment?.email ?? initialValues?.payment?.payoutEmail ?? '',
    venmoHandle: initialValues?.payment?.venmoHandle ?? '',
    mailingAddress: initialValues?.payment?.mailingAddress ?? ''
  });
