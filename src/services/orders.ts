...
const COUNTER_DOC = firestore?.collection('metadata').doc('counters') ?? null;
let demoCounter = demoOrders.length + 1000;

function canUseFirestore() {
  return Boolean(firestore && COUNTER_DOC);
}

async function nextSequentialOrderNumber(): Promise<string> {
  if (!canUseFirestore()) {
    demoCounter += 1;
    return `SHC-${demoCounter.toString().padStart(5, '0')}`;
  }
...
export async function updateOrderStatus(orderId: string, update: OrderStatusUpdate, actor: string): Promise<void> {
  if (!canUseFirestore()) {
    console.warn('Skipping order status update because Firestore is not configured');
    return;
  }
